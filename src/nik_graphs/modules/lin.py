...
def run_path(path, outfile):
    ...
    if eval_all_embeddings:
        df = lin_other_embeddings(embeddings_dir, labels, **indd, **kwargs)
...
def lin_other_embeddings(embeddings_dir, labels, **kwargs):
    ...
    scores = [lin(npz[k], labels, **kwargs) for k in other_embks]
