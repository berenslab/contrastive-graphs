...
def deplist(dispatch: Path):
    ...
    return ["../dataframes/random_inits.h5", "table/dataset_info.parquet"]
...
def plot(h5, df, outfile, format="pdf"):
    ...
    dataset_keys = [k for k in df.sort("n_edges")["dataset_key"] if k in h5]
    fig = plt.figure(
        figsize=(3.35, 1 * len(dataset_keys)),
